#!/usr/bin/env npx tsx
/**
 * Audit local Postalocity prerender builds (dist/index.html) vs live sites.
 *
 * Usage:
 *   npx tsx scripts/audit-postalocity-live.ts
 *   npx tsx scripts/audit-postalocity-live.ts --json
 *
 * Writes: logs/postalocity-live-audit.md
 */
...
const MAPPING_PATH = path.join(__dirname, 'postalocity-live-urls.json');
...
async function fetchLive(url: string): Promise<{ status: number; html: string }> {
  const res = await fetch(url, {
    headers: { 'User-Agent': 'postalocity-audit/1.0', 'Cache-Control': 'no-cache' },
    redirect: 'follow',
  });
...
async function main(): Promise<void> {
  ...
  console.log(`\n🔍 Auditing ${services.length} Postalocity sites vs live...\n`);
  ...
  const reportPath = path.join(LOGS_DIR, 'postalocity-live-audit.md');
