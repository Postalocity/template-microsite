#!/usr/bin/env npx tsx
/**
 * Generate + build + prerender for all Postalocity microsites (index.html deploy).
 *
 * Postalocity sites deploy dist/index.html + dist/assets/ — not shopify.html.
 *
 * Usage:
 *   npx tsx scripts/build-postalocity.ts
 *   npx tsx scripts/build-postalocity.ts --service credit-repair
 *   npx tsx scripts/build-postalocity.ts --skip-generate
 */
...
async function main(): Promise<void> {
  ...
  console.log(`\n🏗️  Postalocity batch build (${services.length} site(s)) — index.html prerender\n`);
  ...
  for (const service of services) {
    ...
    try {
      ...
      // index.html deploy: build + prerender only (skip shopify post-build)
