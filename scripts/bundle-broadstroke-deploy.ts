#!/usr/bin/env npx tsx
/**
 * Bundle built Broadstroke sites for index.html deploy.
 *
 * Usage:
 *   npx tsx scripts/bundle-broadstroke-deploy.ts
 *   npx tsx scripts/bundle-broadstroke-deploy.ts --service mailing
 */
...
function bundleService(service: string): { ok: boolean; message: string } {
  ...
  if (!fs.existsSync(distIndex)) {
    return { ok: false, message: 'missing dist/index.html — run build:broadstroke first' };
  }
  ...
  const checklist = [
    ...
    '',
    '## Live target',
    live?.liveUrl ?? '(map in broadstroke-live-urls.json)',
    live?.notes ? `- Notes: ${live.notes}` : '',
    '',
    '## Steps',
    '1. Upload everything in this folder to the hosting path (e.g. `/mailing/`)',
    '2. Ensure `assets/` paths resolve relative to index.html',
    '3. View-source: confirm #root has prerendered section HTML',
    '4. Run `npm run audit:broadstroke-live`',
    '',
  ].join('\n');
...
function main(): void {
  ...
  console.log(`\n📦 Bundling ${services.length} Broadstroke deploy package(s) → deploy/${BRAND}/\n`);
  ...
  fs.writeFileSync(
    ...
    [
      '# Broadstroke — index.html Deploy Bundles',
      '',
      'Each subdirectory contains prerendered `index.html` + `assets/` for static hosting.',
      '',
      'Workflow: `npm run build:broadstroke` → `npm run bundle:broadstroke-deploy` → upload → `npm run audit:broadstroke-live`',
      '',
    ].join('\n')
  );
