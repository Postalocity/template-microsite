...
export interface ThemeConfig {
  ...
  colors: {
    ...
    heroBg: BrandColors;
    heroForeground: { h: number; s: number; l: number };
 ...
 */
export function getThemeForBrand(brandId: string): ThemeConfig {
  const themes = loadThemes();
  
  // Try to find brand-specific theme
  const brandTheme = Object.values(themes.themes).find(
    t => t.brandId === brandId
  );
 ...
 */
export function listThemes(): { key: string; name: string; brandId: string; description: string }[] {
  const themes = loadThemes();
